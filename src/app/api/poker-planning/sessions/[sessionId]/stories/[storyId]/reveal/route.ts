...
import { revealVotes } from "@/lib/services/poker-planning/session";
...
export async function POST(_: Request, { params }: { params: Params }) {
  try {
    ...
    await revealVotes({ sessionId, storyId, actorUserId: user.id, companyId: user.companyId });
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Failed to reveal votes" }, { status: 400 });
  }
}
