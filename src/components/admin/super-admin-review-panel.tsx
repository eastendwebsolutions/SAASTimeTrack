...
export type SuperAdminEntryRow = {
  id: string;
...
export type SuperAdminTimesheetRow = {
  id: string;
  companyId: string;
  userId: string;
  status: string;
...
type TimesheetsSectionProps = {
  users: SuperAdminUserRow[];
  companies: SuperAdminCompanyRow[];
  projects: ProjectRow[];
  entries: SuperAdminEntryRow[];
  submittedSheets: SuperAdminTimesheetRow[];
};
...
export function SuperAdminUsersSection({ users, companies, workspaceAdmins }: UsersSectionProps) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-zinc-400">Manage roles, Poker Planning admin, and account access across all companies.</p>
...
export function SuperAdminTimesheetsSection({ users, companies, projects, entries, submittedSheets }: TimesheetsSectionProps) {
  ...
  const entriesBySheet = useMemo(() => {
    const map = new Map<string, SuperAdminEntryRow[]>();
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-zinc-400">Review submitted and draft timesheets, then approve or open full detail.</p>
              ...
              {sheetEntries.map((entry) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <span className="break-words text-zinc-200">{projectMap.get(entry.projectId) ?? entry.projectId}</span>
                  </div>
                  <div>
      ...
      {filteredSheets.length === 0 ? (
        <p className="text-sm text-zinc-500">No timesheets match current filters.</p>
      ) : null}
    </div>
  );
}
